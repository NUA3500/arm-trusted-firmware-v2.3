...
/*********************************************************/
int nua3500_nand_wait_ready(unsigned long delay)
{
#if 0
	uint64_t timeout;
...
int nua3500_nand_reset(void)
{
	...
	if (nua3500_nand_wait_ready(1))
		return -ETIMEDOUT;
	else
		return 0;
}

static void nua3500_nand_read_oob(struct nua3500_nand_info *nand, unsigned int page)
{
	int volatile i;
	unsigned char *ptr;
...
static void nua3500_correct_data(unsigned char ucFieidIndex, unsigned char ucErrorCnt, unsigned char* pDAddr)
{
...
static int nua3500_nand_read_page(struct nua3500_nand_info *nand, unsigned int page, uintptr_t buffer)
{
	...
	if (nand->max_bit_corr != 0)    /* ECC_EN */
		nua3500_nand_read_oob(nand, page);
				...
				for (j=0; j<uLoop; j++) {
					...
					for (i=1; i<5; i++) {
						...
						if ((uF1_status & 0x3)==0x01) {
							...
							nua3500_correct_data(j*4+i, uErrorCnt, (unsigned char*)(long)buffer);
							break;
...
static int nua3500_nand_read_pages(struct nua3500_nand_info *nand, uintptr_t buf, int page_start, int page_count)
{
	while (page_count > 0)
	{
		if (nua3500_nand_read_page(nand, page_start, buf)) {
			return -EBADMSG;
		}
		buf += nand->page_size;
		page_count--;
...
static int nua3500_block_isbad(struct nua3500_nand_info *nand, unsigned int pba)
{
	...
	nua3500_nand_read_oob(nand, page+1);
	data = mmio_read_32(REG_NANDRA0) & 0xff;
	if (data != 0xFF)
		return 1;   /* invalid */

	nua3500_nand_read_oob(nand, page+nand->pages_per_block-1);
	data = mmio_read_32(REG_NANDRA0) & 0xff;
	if (data != 0xFF)
		return 1;   /* invalid */

	nua3500_nand_read_oob(nand, page+nand->pages_per_block-2);
	data = mmio_read_32(REG_NANDRA0) & 0xff;
	if (data != 0xFF)
		return 1;   /* invalid */
...
static size_t parse_nand_read(struct nua3500_nand_info *nand, int lba, uintptr_t buf, size_t size)
{
	...
	while (pages_to_read) {
		ret = nua3500_block_isbad(nand, block);
		if (ret) {
			block++;
			if ((--block_count) <= 0)
				goto out;
		...
		ret = nua3500_nand_read_pages(nand, p, block * pages_per_block + page, page_count);
		if (ret)
			goto out;
...
int nua3500_nand_ctrl_init(void)
{
	/* enable nand clock */
...
static void nua3500_nand_setup(struct nua3500_nand_info *nand)
{
	unsigned int reg = 0x00910090; // REG_NANDCTL default value
	int node;

	nua3500_nand_ctrl_init();
	...
	default:
		WARN("NUA3500 NAND Controller is not support this PAGE SIZE. (%d)\n", nand->page_size);
	}
...
static size_t nua3500_nand_read(int lba, uintptr_t buf, size_t size)
{
	size_t count = 0;

	inv_dcache_range(buf, size);

	count = parse_nand_read(&nua3500_nand, lba, buf, size);
...
{
	nua3500_nand_setup(&nua3500_nand);
	*offset = nua3500_nand.offset;

	nand_dev_spec.block_size = nua3500_nand.page_size;
