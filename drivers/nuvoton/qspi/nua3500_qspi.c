...
#include "nua3500_qspi.h"


struct nua3500_qspi_info {
	int page_size;
	int oob_size;
	int pages_per_block;
...
struct nua3500_qspi_info nua3500_qspi;
...
static void nua3500_spinand_reset()
{
	unsigned char cmd[1];
...
static int nua3500_spinand_getstatus(unsigned char offset)
{
...
static void nua3500_spinand_setstatus(unsigned char offset, unsigned char value)
{
	unsigned char cmd[4];
...
static void nua3500_spinand_quad_disable()
{
	unsigned int status;

	/* disable quad mode */
	status = nua3500_spinand_getstatus(0xb0);
	nua3500_spinand_setstatus(0xb0, status & 0xfe);
}

static void nua3500_spinand_quad_enable()
{
	unsigned int status;

	/* enable quad mode */
	status = nua3500_spinand_getstatus(0xb0);
	nua3500_spinand_setstatus(0xb0, status | 0x01);
}

static int nua3500_spinand_singleread(struct nua3500_qspi_info *spinand, unsigned int addr, unsigned int *buf)
{
	unsigned char cmd[4];
	int volatile i;
	...
	while(nua3500_spinand_getstatus(0xc0) & 0x1);    // wait ready
	if (nua3500_spinand_getstatus(0xc0) & 0x20)
	{
		ERROR("Err-ECC\n");
		return 1;
	}
...
static int nua3500_spinand_read_oob(struct nua3500_qspi_info *spinand, unsigned int addr, unsigned int len, unsigned char *buf)
{
	unsigned char cmd[4];
...
static int nua3500_spinand_quadread(struct nua3500_qspi_info *spinand, unsigned int addr, unsigned int *buf)
{
	int volatile i;
	unsigned char cmd[4];
...
static int nua3500_spinand_read_pages(struct nua3500_qspi_info *spinand, uintptr_t buf, int page_start, int page_count)
{
	while (page_count > 0)
	{
		if (spinand->is_quad) {
			if (nua3500_spinand_quadread(spinand, page_start, (unsigned int *)buf)) {
				return -EBADMSG;
			}
		} else {
			if (nua3500_spinand_singleread(spinand, page_start, (unsigned int *)buf)) {
				return -EBADMSG;
			}
		}
...
static int nua3500_spinand_block_isbad(struct nua3500_qspi_info *spinand, unsigned int block)
{
	unsigned int page = block * spinand->pages_per_block;
	unsigned char data;

	nua3500_spinand_read_oob(spinand, page, 1, &data);
	if (data != 0xff)
		return 1;   /* bad block */

	nua3500_spinand_read_oob(spinand, page+1, 1, &data);
	if (data != 0xff)
		return 1;   /* bad block */
...
static size_t parse_spinand_read(struct nua3500_qspi_info *spinand, int lba, uintptr_t buf, size_t size)
{
	...
	while (pages_to_read) {
		ret = nua3500_spinand_block_isbad(spinand, block);
		if (ret) {
			block++;
			if ((--block_count) <= 0)
				goto out;
		...
		ret = nua3500_spinand_read_pages(spinand, p, block * pages_per_block + page, page_count);
		if (ret)
			goto out;
...
static void nua3500_spinand_setup(struct nua3500_qspi_info *spinand)
{
	int node;
	...
	node = fdt_node_offset_by_compatible(fdt, -1, "nuvoton,nua3500-qspi");
	if (node < 0) {
		WARN("The compatible property `nuvoton,nua3500-qspi` not found\n");
	}
	...
	nua3500_spinand_quad_disable();
	/* reset spi nand */
	nua3500_spinand_reset();
	while(1)
	{
		if ((nua3500_spinand_getstatus(0xc0) & 0x1) == 0)
			break;
	}

	if (spinand->is_quad) {
		nua3500_spinand_quad_enable();
	}
}

/***************************************************************/
static int nua3500_spinor_reset(void)
{
	/* reset SPI flash */
	mmio_write_32(REG_QSPI0_SSCTL, 0x01);   // CS0 low
...
int nua3500_spinor_quad_enable(struct nua3500_qspi_info *spinor)
{
	unsigned char data[2];
	unsigned char cmd[4];
...
int spinor_quad_read(struct nua3500_qspi_info *spinor, unsigned int addr, unsigned int len, unsigned int *buf)
{
	int volatile i;
	int volatile count;

	nua3500_spinor_quad_enable(spinor);
	...
	// disable quad mode
	nua3500_spinor_reset();

	return 0;
}


static size_t parse_spinor_read(struct nua3500_qspi_info *spinor, int lba, uintptr_t buf, size_t size)
{
	if (spinor->is_quad)
...
static void nua3500_spinor_setup(struct nua3500_qspi_info *spinor)
{
	int node;
...
/***************************************************************/
static size_t nua3500_spinand_read(int lba, uintptr_t buf, size_t size)
{
	size_t count = 0;

	inv_dcache_range(buf, size);

	count = parse_spinand_read(&nua3500_qspi, lba, buf, size);
...
{
	nua3500_qspi.is_quad = is_quad;
	nua3500_spinand_setup(&nua3500_qspi);
	*offset = nua3500_qspi.offset;

	spinand_dev_spec.block_size = nua3500_qspi.page_size;
...
static size_t nua3500_spinor_read(int lba, uintptr_t buf, size_t size)
{
	size_t count = 0;

	inv_dcache_range(buf, size);

	count = parse_spinor_read(&nua3500_qspi, lba, buf, size);
...
{
	nua3500_qspi.is_quad = is_quad;
	nua3500_spinor_setup(&nua3500_qspi);
	*offset = nua3500_qspi.offset;
