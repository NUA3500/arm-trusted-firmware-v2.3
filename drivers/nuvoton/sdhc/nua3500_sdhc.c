...
static int nua3500_sdhc_hw_init(struct mmc *mmc)
{
	struct mmc_cmd cmd;
	int err;
...
static void nua3500_sdhc_setup(struct mmc *mmc)
{
	int node;

	nua3500_sdhc_hw_init(mmc);
...
static size_t nua3500_sdhc_read(int lba, uintptr_t buf, size_t size)
{
	size_t count = 0;

	inv_dcache_range(buf, size);

	if (sdh_read_blocks(&nua3500_mmc, lba, (void *)buf, size/512) == 0)
		count = size;
...
int nua3500_sdhc_init(struct io_block_dev_spec **block_dev_spec, long *offset, int sdhc)
{
	if (sdhc == 0) {
		...
		nua3500_mmc.base = SDH1_BASE;
	}

	nua3500_sdhc_setup(&nua3500_mmc);
	*offset = nua3500_mmc.offset;
