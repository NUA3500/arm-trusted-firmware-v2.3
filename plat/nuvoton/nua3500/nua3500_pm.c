...
/*******************************************************************************
 * Function which implements the common NUA3500 specific operations to power down a
 * cluster in response to a CPU_OFF or CPU_SUSPEND request.
 ******************************************************************************/
#if 0
static void nua3500_cluster_pwrdwn_common(void)
{
 ...
 * on ARM GICv3 implementations on NUA3500. This is required, because NUA3500 does not
...
static void nua3500_power_domain_on_finish_common(const psci_power_state_t *target_state)
{
#if 0
	unsigned long mpidr;
	...
	if (target_state->pwr_domain_state[ARM_PWR_LVL1] ==
		 ...
		 * attempting to power down. In this case the NUA3500 power
...
/*******************************************************************************
 * NUA3500 handler called when a CPU is about to enter standby.
 ******************************************************************************/
...
/*******************************************************************************
 * NUA3500 handler called when a power domain is about to be turned on. The
...
/*******************************************************************************
 * NUA3500 handler called when a power domain is about to be turned off. The
...
static void nua3500_pwr_domain_off(const psci_power_state_t *target_state)
{
	...
	if (target_state->pwr_domain_state[ARM_PWR_LVL1] ==
					ARM_LOCAL_STATE_OFF)
		nua3500_cluster_pwrdwn_common();
#endif
}

/*******************************************************************************
 * NUA3500 handler called when a power domain is about to be suspended. The
 * target_state encodes the power state that each level should transition to.
 ******************************************************************************/
static void nua3500_pwr_domain_suspend(const psci_power_state_t *target_state)
{
	...
	/*
	 * NUA3500 has retention only at cpu level. Just return
...
/*******************************************************************************
 * NUA3500 handler called when a power domain has just been powered on after
 * being turned off earlier. The target_state encodes the low power state that
 * each level has woken up from.
 ******************************************************************************/
static void nua3500_pwr_domain_on_finish(const psci_power_state_t *target_state)
{
	nua3500_power_domain_on_finish_common(target_state);

}

/*******************************************************************************
 * NUA3500 handler called when a power domain has just been powered on and the cpu
 ...
 ******************************************************************************/
static void nua3500_pwr_domain_on_finish_late(const psci_power_state_t *target_state)
{
 ...
 ******************************************************************************/
static void nua3500_pwr_domain_suspend_finish(const psci_power_state_t *target_state)
{
#if 0
	/*
...
/*******************************************************************************
 * NUA3500 handlers to shutdown/reboot the system
 ******************************************************************************/
static void __dead2 nua3500_system_off(void)
{
#if 0
...
#endif
	wfi();
	ERROR("NUA3500 System Off: operation not handled.\n");
	panic();
}

static void __dead2 nua3500_system_reset(void)
{
#if 0
	/* Write the System Configuration Control Register */
	mmio_write_32(V2M_SYSREGS_BASE + V2M_SYS_CFGCTRL,
...
#endif
	wfi();
	ERROR("NUA3500 System Reset: operation not handled.\n");
	panic();
}

static int nua3500_node_hw_state(u_register_t target_cpu,
			     unsigned int power_level)
{
	...
	/*
	 * Read the status of the given MPDIR from NUA3500 power controller. The
...
/*
 * The NUA3500 doesn't truly support power management at SYSTEM power domain. The
 ...
 * save and restore sequences on NUA3500.
 */
#if !ARM_BL31_IN_DRAM
static void nua3500_get_sys_suspend_power_state(psci_power_state_t *req_state)
{
	unsigned int i;
...
/*
 * Custom `translate_power_state_by_mpidr` handler for NUA3500. Unlike in the
 * `nua3500_validate_power_state`, we do not downgrade the system power
 * domain level request in `power_state` as it will be used to query the
 * PSCI_STAT_COUNT/RESIDENCY at the system power domain level.
 */
static int nua3500_translate_power_state_by_mpidr(u_register_t mpidr,
...
plat_psci_ops_t plat_arm_psci_pm_ops = {
	.cpu_standby = nua3500_cpu_standby,
	.pwr_domain_on = nua3500_pwr_domain_on,
	.pwr_domain_off = nua3500_pwr_domain_off,
	.pwr_domain_suspend = nua3500_pwr_domain_suspend,
	.pwr_domain_on_finish = nua3500_pwr_domain_on_finish,
	.pwr_domain_on_finish_late = nua3500_pwr_domain_on_finish_late,
	.pwr_domain_suspend_finish = nua3500_pwr_domain_suspend_finish,
	.system_off = nua3500_system_off,
	.system_reset = nua3500_system_reset,
	.validate_power_state = nua3500_validate_power_state,
//	.validate_ns_entrypoint = arm_validate_psci_entrypoint,
	.translate_power_state_by_mpidr = nua3500_translate_power_state_by_mpidr,
	.get_node_hw_state = nua3500_node_hw_state,
#if !ARM_BL31_IN_DRAM
	/*
	 ...
	 */
	.get_sys_suspend_power_state = nua3500_get_sys_suspend_power_state,
#endif
