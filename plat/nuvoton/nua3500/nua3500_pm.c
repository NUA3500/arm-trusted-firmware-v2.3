...
#define NUA3500_CORE_PWR_STATE(state) \
    ((state)->pwr_domain_state[MPIDR_AFFLVL0])
#define NUA3500_CLUSTER_PWR_STATE(state) \
    ((state)->pwr_domain_state[MPIDR_AFFLVL1])
#define NUA3500_SYSTEM_PWR_STATE(state) \
...
static __inline void nua3500_LockReg(void)
{
	mmio_write_32(0x404601A0, 0);
}

// Q ch
void nua3500_ddr_hw_pd(void)
{
	int DDR_QCH_BPPORT0;
	int DDR_QCH_BPPORT1;
	int DDR_QCH_BPPORT2;
						...
						psci_power_state_t * target_state)										
{
	...
	if (NUA3500_SYSTEM_PWR_STATE(target_state) == PLAT_MAX_OFF_STATE)
		...
		while (1) {
			wfi();
			((void (*)(void))nua3500_sec_entrypoint) ();
		}
	}
}
...
plat_psci_ops_t plat_arm_psci_pm_ops = {
	...
	.validate_ns_entrypoint = nua3500_validate_ns_entrypoint,
	...
	.pwr_domain_pwr_down_wfi = nua3500_pwr_domain_pwr_down_wfi,
